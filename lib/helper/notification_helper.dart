...
import 'package:wekala_delivery/common/widgets/level_congratulations_dialog_widget.dart';
...
import 'package:wekala_delivery/features/dashboard/screens/dashboard_screen.dart';
...
import 'package:wekala_delivery/features/ride_module/add_vehicle/screens/vehicle_details_screen.dart';
import 'package:wekala_delivery/features/ride_module/ride_order/screens/pending_ride_list_screen.dart';
import 'package:wekala_delivery/features/splash/controllers/splash_controller.dart';
import 'package:wekala_delivery/helper/custom_print_helper.dart';
...
import 'package:wekala_delivery/features/my_account/screens/my_account_screen.dart';
import 'package:wekala_delivery/features/delivery_module/order/screens/order_request_screen.dart';
import 'package:wekala_delivery/features/delivery_module/order/screens/order_screen.dart';
...
import 'package:wekala_delivery/features/ride_module/map/screens/map_screen.dart';
import 'package:wekala_delivery/features/ride_module/review/screens/review_screen.dart';
import 'package:wekala_delivery/features/ride_module/ride_order/controllers/ride_controller.dart';
import 'package:wekala_delivery/features/ride_module/ride_order/screens/ongoing_ride_list_screen.dart';
import 'package:wekala_delivery/features/ride_module/safety/controllers/safety_alert_controller.dart';
import 'package:wekala_delivery/helper/ride_helper.dart';
import 'package:wekala_delivery/features/ride_module/ride_order/screens/ride_details_screen.dart';
...
class NotificationHelper {
  static Future<void> initialize(
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      bool pusherDisconnected =
          Get.find<SplashController>().pusherConnectionStatus == null ||
          Get.find<SplashController>().pusherConnectionStatus ==
              'Disconnected' ||
          (Get.find<SplashController>().configModel?.webSocketStatus == false);
      if (message.data['type'] == 'message' &&
        ...
        if (message.data['type'] == 'message' &&
            Get.currentRoute.startsWith('/MapScreen')) {
      ...
      if (message.data['action'] == "driver_new_ride_request" &&
          pusherDisconnected) {
        print(
          "Connected =====> Connection get from Notification/ Current Pusher Status=====> ${Get.find<SplashController>().pusherConnectionStatus}",
        );

        _whenNewRequestFound(message);
      } else if (message.data['action'] == "driver_trip_request_canceled" &&
          pusherDisconnected) {
      ...
      } else if (message.data['action'] == "driver_customer_canceled_trip" &&
          pusherDisconnected) {
        print(
          "Connected =====> Connection get from Notification/ Current Pusher Status=====> ${Get.find<SplashController>().pusherConnectionStatus}",
        );
      ...
      } else if ((message.data['action'] == "customer_trip_completed" ||
              message.data['action'] == "customer_trip_canceled" ||
        ...
        Get.find<RideController>()
            .getRideDetails(message.data['ride_request_id'], reload: true)
            .then((v) {
              Get.offAll(
                () =>
                    RideDetailsScreen(rideId: message.data['ride_request_id']),
              );
            });
      } else if (message.data['type'] != 'message' &&
          checkContainsAction(message.data['action'])) {
      ...
      } else if (message.data['action'] == "other_withdraw_request_rejected" ||
          message.data['action'] == "other_withdraw_request_approved") {
        Get.find<ProfileController>().getProfile();
        ...
        // Get.offAll(const DashboardScreen());
        Get.offAllNamed(RouteHelper.getInitialRoute());
      } else if (message.data['action'] == 'other_referral_reward_received' &&
      ...
      } else if (message.data['action'] == "customer_payment_successful" &&
          pusherDisconnected) {
        print(
          "Connected =====> Connection get from Notification/ Current Pusher Status=====> ${Get.find<SplashController>().pusherConnectionStatus}",
        );
      ...
      } else if (message.data['action'] == "customer_another_driver_assigned" &&
          pusherDisconnected) {
        print(
          "Connected =====> Connection get from Notification/ Current Pusher Status=====> ${Get.find<SplashController>().pusherConnectionStatus}",
        );
      ...
      try {
        if (message.data.isNotEmpty) {
          ...
          final Map<NotificationType, Function> notificationActions = {
            ...
            NotificationType.ride_request: () => Get.to(
              () => RideDetailsScreen(
                rideId: notificationBody.rideRequestId ?? "",
              ),
            ),
          };
  ...
  static Future<void> notificationToRoute(
    ...
    if (data.action == "driver_new_ride_request") {
      Get.find<RideController>().ongoingTripList().then((value) {
        if ((Get.find<RideController>().ongoingRide ?? []).isEmpty) {
          Get.find<RideController>()
              .getRideDetailBeforeAccept(data.rideRequestId!)
              .then((value) {
      ...
      ///Bid Ride Accepted in this case....
      Get.find<RideController>().getRideDetails(data.rideRequestId!).then((
        value,
      ) {
        if (value != null) {
          Get.find<RiderMapController>().setRideCurrentState(
    ...
    } else if (data.action == "customer_payment_successful" &&
        data.action == "ride_request") {
      Get.find<RideController>().getRideDetails(data.rideRequestId!).then((
        value,
      ) {
    ...
    } else if (data.action == "driver_customer_canceled_trip" ||
        data.action == "customer_another_driver_assigned") {
    ...
    } else if (data.action == "other_withdraw_request_rejected" ||
        data.action == "other_withdraw_request_approved" ||
        data.action == "other_admin_collected_cash" ||
        data.action == "other_withdraw_request_reversed") {
      // Get.offAllNamed(RouteHelper.getInitialRoute());
    ...
    //   Get.find<BottomMenuController>().setTabIndex(3);
    // }
    else if (data.action == 'driver_tips_from_customer' ||
        data.action == 'customer_payment_successful') {
      _toRoute(formSplash, RideDetailsScreen(rideId: data.rideRequestId!));
    } else if (data.action == 'other_safety_problem_resolved' &&
        data.action == 'other_safety_alert_sent') {
      Get.find<RideController>().getRideDetails(data.rideRequestId!).then((
        value,
      ) {
        if (value != null) {
          if (Get.find<RideController>().tripDetail?.currentStatus ==
            ...
            if (Get.currentRoute != '/RideDetailsScreen') {
              _toRoute(
                formSplash,
                RideDetailsScreen(rideId: data.rideRequestId!),
              );
            }
          }
        }
  ...
  static void _whenNewRequestFound(RemoteMessage message) {
    Get.find<RideController>().ongoingTripList().then((value) {
      print(
        '======here======0 ?? >> ${Get.find<RideController>().ongoingRide}',
      );
      if ((Get.find<RideController>().ongoingRide ?? []).isEmpty) {
        print('======here======1');
        Get.find<RideController>().getPendingRideRequestList(1);
  ...
  static bool checkContainsAction(String action) {
    List<String> actions = [
      'driver_registration_vehicle_request_approved',
      'driver_registration_vehicle_request_denied',
  ...
  static void _whenCustomerCancelTrip(
    RemoteMessage message, {
    bool isAnotherDriverAssigned = false,
  }) {
