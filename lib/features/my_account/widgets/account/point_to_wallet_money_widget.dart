...
class _PointToWalletMoneyWidgetState extends State<PointToWalletMoneyWidget> {
  ...
  void initState() {
    amountController.text =
        Get.find<ProfileController>().profileModel?.loyaltyPoint.toString() ??
        '0';
    super.initState();
  }
    ...
    return Dialog(
      ...
      child: GetBuilder<MyAccountController>(
        builder: (myAccountController) {
          ...
          final int minPoint = isRideActive
              ? (config?.riderLoyalityPointData?.minLoyalityPointToConvert ?? 0)
              : (config?.dmLoyalityPointData?.minLoyalityPointToConvert ?? 0);
          final int availablePoint =
              Get.find<ProfileController>().profileModel?.loyaltyPoint ?? 0;
          final int conversionRate = isRideActive
              ? (config?.riderLoyalityPointData?.loyalityPointConversionRate ??
                    1)
              : (config?.dmLoyalityPointData?.loyalityPointConversionRate ?? 1);
          final bool isValid =
              enteredPoint >= minPoint &&
              enteredPoint <= availablePoint &&
              enteredPoint > 0;
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  '${conversionRate}pt = '
                  '${Get.find<SplashController>().configModel?.currencySymbol}1',
                  style: robotoMedium.copyWith(
                ...
                IntrinsicWidth(
                  child: TextFormField(
                    ...
                    style: robotoBold.copyWith(
                      color: enteredPoint <= availablePoint
                ...
                Text(
                  ...
                  '${PriceConverterHelper.convertPrice(enteredPoint / conversionRate)}',
                  style: robotoRegular,
                ),
                ...
                if (enteredPoint < minPoint)
                  _errorText(
                    '${'conversion_point_must_be_at_least'.tr} $minPoint',
                  )
                else if (enteredPoint > availablePoint)
                  _errorText(
                    '${'you_can_convert_up_to'.tr} $availablePoint ${'points'.tr}',
                  ),
                            ...
                            myAccountController
                                ...
                                .then((value) {
                                  if (value.statusCode == 200) {
                                    Get.back();
                                    showCustomSnackBar(
                                      'successfully_converted_to_wallet_money'
                                          .tr,
                                      isError: false,
                                    );
                                  }
