...
import 'package:wekala_delivery/features/my_account/widgets/custom_drop_down.dart';
import 'package:wekala_delivery/features/my_account/widgets/success_dialog_widget.dart';
...
class _WithdrawBottomSheetState extends State<WithdrawBottomSheet> {
  ...
  final TextInputFormatter _amountInputFormatter =
  ...
  void initState() {
    ...
    Get.find<DisbursementController>().updateSelectedWithdrawMethod(
      shouldUpdate: false,
    );
                ...
                children: [
                  ...
                  CustomDropDown(
                    ...
                    onChange: (int? value, int index) {
                      disburseController.updateSelectedWithdrawMethod(
                        newValue: withdrawList?[index],
                      );
                    },
                  ),
                  const SizedBox(height: Dimensions.paddingSizeDefault),

                  disburseController.selectedWithdrawMethod != null
                      ? Container(
                          decoration: BoxDecoration(
                  ...
                  CustomButtonWidget(
                    ...
                    onPressed:
                        widget.withdrawAmount >=
                            _convertDouble(amountController.text)
                        ? () async {
                            if (disburseController.selectedWithdrawMethod ==
                                null) {
                              showCustomSnackBar("select_withdrawal_method".tr);
                            } else if (_convertDouble(amountController.text) <=
                                0) {
                              showCustomSnackBar(
                                "amount_should_not_be_zero".tr,
                              );
                            } else {
                              Map<String, dynamic> data = {};
                              data['id'] = disburseController
                                  .widthDrawMethods?[disburseController
                                      .selectedMethodIndex!]
                                  .id
                                  .toString();
                              double amountValue =
                              ...
                              for (
                                int i = 0;
                                i < disburseController.methodFields.length;
                                i++
                              ) {
                                var result = disburseController.methodFields[i];

                                if (disburseController
                                            .selectedWithdrawMethod
                                            ?.methodFields !=
                                        null &&
                                    disburseController
