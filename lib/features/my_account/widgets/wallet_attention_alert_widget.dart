...
class WalletAttentionAlertWidget extends StatelessWidget {
  ...
  const WalletAttentionAlertWidget({
    super.key,
    required this.isOverFlowBlockWarning,
  });
                        ...
                        children: [
                          TextSpan(
                            recognizer: TapGestureRecognizer()
                              ..onTap = () {
                                bool isRideActive =
                                    AppConstants.appMode == AppMode.ride;
                                final config =
                                    Get.find<SplashController>().configModel;
                                double minimumPayableAmount = isRideActive
                                    ? (config?.minAmountToPayRider ?? 0)
                                    : (config?.minAmountToPayDm ?? 0);
                                  ...
                                  if (Get.find<SplashController>()
                                    ...
                                    showCustomSnackBar(
                                      ...
                                      profileController
                                          ...
                                          .payableBalance!) {
                                    showCustomSnackBar(
                                      '${'you_do_not_have_sufficient_balance_to_pay_the_minimum_payable_balance_is'.tr} ${PriceConverterHelper.convertPrice(minimumPayableAmount)}',
                                    );
                                  }
                                }
                              },
