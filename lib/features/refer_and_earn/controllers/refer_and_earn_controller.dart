...
import 'package:wekala_delivery/features/refer_and_earn/domain/models/refer_and_earn_model.dart';
...
class ReferAndEarnController extends GetxController implements GetxService {
  ...
  Future<Response> getEarningHistoryList(int offset) async {
    ...
    Response response = await referEarnServiceInterface.getEarningHistoryList(
      offset,
      _dateRange ?? 'this_year',
      _startDate,
      _endDate,
    );
    if (response.statusCode == 200) {
      ...
      if (offset == 1) {
        ...
        referralModel!.refrealEarnings!.addAll(
          ReferEarningModel.fromJson(response.body).refrealEarnings!,
        );
        referralModel!.offset = ReferEarningModel.fromJson(
          response.body,
        ).offset;
