...
class HomeScreenHelper {
  void pendingListPusherImplementation() {
    for (
      int i = 0;
      i < Get.find<RideController>().pendingRideRequestModel!.data!.length;
      i++
    ) {
      PusherHelper().customerInitialTripCancel(
        Get.find<RideController>().pendingRideRequestModel!.data![i].id!,
        Get.find<ProfileController>().profileModel!.id!.toString(),
      );
  ...
  void ongoingLastRidePusherImplementation() {
    ...
    for (int i = 0; i < Get.find<RideController>().ongoingRide!.length; i++) {
      if (data.contains(
        Get.find<RideController>().ongoingRide![i].currentStatus,
      )) {
        PusherHelper().tripCancelAfterOngoing(
          Get.find<RideController>().ongoingRide![i].id!,
        );
        PusherHelper().customerInitialTripCancel(
          Get.find<RideController>().ongoingRide![i].id!,
          Get.find<ProfileController>().profileModel!.id!.toString(),
