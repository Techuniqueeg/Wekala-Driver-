...
class EarningReportRepository implements EarningReportRepositoryInterface {
  ...
  Future<EarningReportModel?> getEarningReport({
    ...
    final filterParam =
        (from != null && to != null && from.isNotEmpty && to.isNotEmpty)
        ? '&filter=custom&from=$from&to=$to'
        : '';
    Response response = await apiClient.getData(
      '${isDelivery ? AppConstants.newEarningReportUri : AppConstants.riderEarningReportUri}?limit=10&offset=$offset$filterParam&token=${_getUserToken()}',
    );
